...
- A) Profile picture
- B) Snapshot of configured VM used as template for deploying new VMs
- C) Wallpaper
- D) Icon set
...
- A) General settings
- B) Removing machine-specific information to create reusable image
- C) Making image public
- D) Compression
...
- A) Nothing
- B) VMs from image may have duplicate SIDs, hostname conflicts, licensing issues
- C) Image is better
- D) No problem
...
- A) No versioning
- B) Yes, Azure Compute Gallery supports versioning and rollback
- C) Manual only
- D) Not recommended
...
- B) Yes, Compute Gallery supports multi-region replication
- C) Manual copy only
- D) Not possible
...
- A) Same thing
- B) Snapshot is point-in-time disk copy; image is generalized template for new VMs
- C) Image is smaller
- D) Snapshot is newer
...
- A) Yes, always safe
- B) Not recommended; VM should be stopped and generalized
- C) Only Linux
- D) Only Windows
