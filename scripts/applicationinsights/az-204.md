...
Understanding when to use Application Insights versus other monitoring solutions, Configuring Application Insights for web apps, APIs, and containerized applications, and implementing custom telemetry using the application insights sdk.

**Monitor App Performance**
Using Live Metrics for real-time monitoring, Analyzing application performance and identifying bottlenecks, Understanding Application Map for visualizing dependencies, and tracking custom metrics and events.

**Analyze and Troubleshoot**
Using the Failures view to diagnose issues, Performing end-to-end transaction analysis, Querying telemetry data using Kusto Query Language in Log Analytics, and understanding client-side versus server-side telemetry.
...
**Requests** - HTTP requests to your web application, **Dependencies** - Calls your app makes to external services, databases, or APIs, **Exceptions** - Unhandled exceptions in your application, **Traces** - Log messages from your application code, **Custom Events** - Business events you explicitly track, **Page Views** - Client-side page loads and navigation, and **custom metrics** - numerical measurements you define.
...
Adaptive sampling (adjusts automatically based on volume), Fixed-rate sampling (samples a fixed percentage), and ingestion sampling (applied at the application insights service).
...
The Azure Portal UI, ARM templates or Bicep, Azure CLI commands, and application settings.

**SDK Configuration for .NET Applications**

Understand how to:
Add the NuGet package, Configure the connection string in appsettings.json or environment variables, Initialize the TelemetryClient, and track custom events and metrics.
...
Creating Application Insights resources using the CLI or Portal, Integrating Application Insights into .NET applications, Enabling auto-instrumentation for App Service applications, Navigating the Application Insights UI to diagnose issues, Writing basic Kusto queries in Log Analytics, and configuring alerts based on telemetry data.
