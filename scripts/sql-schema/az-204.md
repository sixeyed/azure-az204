...
- When to use different types of storage access keys
- How Shared Access Signatures provide time-limited access to storage resources
- The structure of blob URLs and how the storage account name forms part of the public address
...
- SQL Server firewall rules at the server and database level
- The special IP address 0.0.0.0 to allow Azure services
- How to restrict access to specific IP ranges for client applications
...
- How to create and configure SQL Server and databases using the Azure CLI
- Connection string formats and authentication methods
- The difference between server-level and database-level administration
...
- **Bacpac files** include schema and data - use these for backups, migrations, and creating complete database copies
- **Dacpac files** include only schema - use these for version control and deploying schema changes in DevOps pipelines
- You can **import** from a Bacpac to create a new database
- You can **export** to a Bacpac to backup an existing database
- You cannot import from a Dacpac - they're used differently, typically with SqlPackage tooling in CI/CD pipelines
...
- Clone a production database to create a test environment
- Move a database between Azure regions
- Restore a database from a backup

The Bacpac import/export process we used is one approach. Other options include:
- Database Migration Service for live migrations
- Transactional replication for continuous sync
- Geo-replication for disaster recovery
- Point-in-time restore for backup scenarios
...
- Azure CLI commands for SQL and Storage
- The Azure Portal for verification and administration
- SQL queries in the browser-based Query Editor
- Working with both structured (SQL) and unstructured (Blob) data services
