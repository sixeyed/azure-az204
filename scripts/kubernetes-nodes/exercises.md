# Kubernetes Nodes - Exercise Narration Script

**Style:** Conversational voice-over for screen recordings
**Pace:** Moderate, allow time for command execution
...
Great! So kubectl get nodes gives us a simple table view. You can see each node listed with some basic information: its name, status, the Kubernetes version it's running, and how long it's been running.

The status column is particularly important - you want to see "Ready" here, which means the node is healthy and able to accept pods.

**[Screen: Point to status column]**

In a production AKS cluster, you might see multiple nodes here. Each one is a virtual machine running in Azure, ready to host your containerized applications.
...
Wow, that's a lot of information! And this is exactly what you'll need when troubleshooting or planning capacity.

Let's break down what we're seeing here:

**[Screen: Scroll through output slowly]**

At the top, you see labels - these are key-value pairs that help organize and identify nodes. You might see labels for the availability zone, instance type, and operating system.
...
**[Screen: Point to capacity section]**
...
This shows you all the available kubectl commands. There are a lot of them, but you'll find yourself using just a handful most of the time.

You can also get help for specific commands. Let's try:

`kubectl get --help`

**[Pause]**

This shows you all the options and parameters for the get command. Notice the examples section at the bottom - these are really helpful when you're learning.
...
Type: `kubectl explain node`

**[Pause]**

Perfect! This gives you documentation right in your terminal. You can see the description of what a node is, what API version it uses, and what fields it contains.

You can drill down into specific fields too. For example, if you wanted to learn about the node's status field, you could type kubectl explain node dot status. This is a great way to explore Kubernetes resources without leaving your terminal.
...
By default, kubectl get shows you a table, but you can request other formats. Let's try JSON.

**[Screen: Terminal]**

First, let's get the name of one of our nodes. Looking at our earlier output, I'll use this one.

**[Note to editor: Use the actual node name from the screen]**

Type: `kubectl get node` and then your node name, followed by `-o json`

**[Pause for execution]**

There we go! Now we're seeing all the node information in JSON format. This is the raw data that Kubernetes stores about this node.

**[Screen: Scroll slowly through JSON]**

You can see the metadata section with labels and annotations, the spec section with node configuration, and the status section with current state information.
...
Let's say we just want to know how many CPU cores this node has. We can use JSON Path for that.

Type: `kubectl get node` and your node name, then `-o jsonpath=` and in quotes: `'{.status.capacity.cpu}'`
...
Let's try it: `kubectl get nodes` - plural - with the same JSON Path.

**[Pause]**

Interesting! When you query multiple nodes, JSON Path concatenates the results. So if you have three nodes with two cores each, you'd see something like: two two two, all run together. Keep this in mind when you're writing queries.
...
The simplest approach is to look at the labels directly. When we ran kubectl describe nodes earlier, the labels were right at the top of the output.

**[Run command and show output]**

See these labels here? They tell us quite a bit. There's usually a label for the architecture - like arm64 or amd64 - and labels for the operating system, like linux or windows.

**[Screen: Point to relevant labels]**

Another approach is to use the get command with label output. Try:

`kubectl get nodes --show-labels`

**[Pause]**

This shows the regular table, but with an additional column containing all the labels.

And if you wanted to use JSON Path to extract just the labels, you could do:

`kubectl get node` your-node-name `-o jsonpath='{.metadata.labels}'`

**[Pause]**

There you go! Multiple ways to solve the same problem. In Kubernetes, there's often more than one way to get the information you need.
